...
9. Run the focused test file and `bun typecheck` from `packages/2cy`.
...
```bash
git grep -n "Effect.runPromise\|ManagedRuntime\|Promise.withResolvers\|Bun.sleep\|withTestInstance" -- packages/2cy/test
```
