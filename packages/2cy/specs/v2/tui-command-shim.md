...
Delete `packages/2cy/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/2cy/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/2cy/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/2cy`
- TUI plugin loader tests in `packages/2cy` if runtime plugin API wiring changed
