...
Trim `packages/2cy/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/2cy/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/2cy`.
- `[x]` `bun typecheck` from `packages/2cy`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/2cy/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/2cy`.
- `[x]` `bun dev generate > /tmp/opencode-openapi.json` from `packages/2cy`.
...
- Edit `packages/2cy/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/opencode-openapi.json` from `packages/2cy`.
...
- Check source schemas in files like `packages/2cy/src/session/schema.ts`, `packages/2cy/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/2cy/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/2cy/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
