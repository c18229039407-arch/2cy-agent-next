...
Practical reference for the current tool-migration state in `packages/2cy`.
...
Tool tests should use the existing Effect helpers in `packages/2cy/test/lib/effect.ts`:
