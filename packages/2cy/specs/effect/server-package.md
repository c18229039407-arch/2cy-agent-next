...
- The server still lives in `packages/2cy`.
...
Until enough shared service code lives outside `packages/2cy`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/2cy`

It should not import `packages/2cy` services while `packages/2cy`
...
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/2cy` implementation details.
