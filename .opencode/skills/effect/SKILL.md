...
- Use `testEffect(...)` from `packages/2cy/test/lib/effect.ts` for tests that exercise Effect services, layers, runtime context, scoped resources, or platform integrations.
...
- Run tests from package directories such as `packages/2cy`; never run package tests from the repo root.
