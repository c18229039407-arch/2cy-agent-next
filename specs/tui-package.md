...
`packages/2cy/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/2cy`,
`packages/cli`, or `@2cy/core`.
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/2cy`.
- Do not replace private `packages/2cy` imports with `@2cy/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@2cy/tui`.
...
```text
packages/2cy/src/cli/cmd/tui
```

Its private dependency on `packages/2cy` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/2cy/src/*`.
...
Status: Completed. The private `@2cy/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@2cy/tui`.
...
- The package has no dependency on `opencode`, `@2cy/cli`, or
  `@2cy/core`.
...
primitives now live in `@2cy/tui`. The legacy host consumes explicit
...
and build inputs through `@2cy/tui/runtime`. Movable app, component,
...
`@2cy/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/2cy` initially.
...
canonical in `@2cy/tui`. Configured references resolve through the new
...
`@2cy/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/2cy`.
...
- The package has no imports from `packages/2cy`, `packages/cli`, or
  `@2cy/core`.
...
public `@2cy/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/2cy`.
- Keep the legacy embedded worker and server startup in `packages/2cy`.
...
- Remove obsolete `@tui/*` path mappings from `packages/2cy`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@2cy/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@2cy/core`, or either
  executable package.
...
rg '@2cy/core|packages/2cy|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/2cy/src packages/2cy/test
```
...
- Build the current-platform `packages/2cy` binary.
