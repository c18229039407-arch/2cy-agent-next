# Remove `packages/2cy/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/2cy/src/storage/db.ts` module.
...
Production imports from `packages/2cy/src/storage/db.ts` are concentrated in 22 source files:

- `packages/2cy/src/account/repo.ts`
- `packages/2cy/src/cli/cmd/db.ts`
- `packages/2cy/src/cli/cmd/import.ts`
- `packages/2cy/src/cli/cmd/stats.ts`
- `packages/2cy/src/control-plane/workspace.ts`
- `packages/2cy/src/index.ts`
- `packages/2cy/src/node.ts`
- `packages/2cy/src/permission/index.ts`
- `packages/2cy/src/project/project.ts`
- `packages/2cy/src/server/projectors.ts`
- `packages/2cy/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/2cy/src/server/shared/fence.ts`
- `packages/2cy/src/session/message-v2.ts`
- `packages/2cy/src/session/projectors.ts`
- `packages/2cy/src/session/prompt.ts`
- `packages/2cy/src/session/session.ts`
- `packages/2cy/src/session/todo.ts`
- `packages/2cy/src/share/share-next.ts`
- `packages/2cy/src/storage/db.ts`
- `packages/2cy/src/sync/index.ts`
- `packages/2cy/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy opencode database wrapper; `packages/2cy/src/storage/db.ts` has been deleted.
...
All migration groups are complete or superseded. `packages/2cy/src/storage/db.ts` has been deleted.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/2cy`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/2cy/src`
- `bun typecheck` from `packages/2cy`
- Relevant package tests from `packages/2cy`, not the repo root
